    ...
    for epoch in range(n_epochs):
        ...
        if epoch % 5 == 0:
            ...
            if score > best_score or dataloader_train.name == "pascalpart":
                ...
                if model.name == "resnet50" or model.name == "vitB-bcos":
                    torch.save(
                        model.state_dict(),
...
def train_dnn(dataset_name: str, model: nn.Module) -> None:
    ...
    if model.name == "vitB-bcos" or model.name == "resnet50":
        metadata = data.metadata_importer(dataset_name)
        model = models.model_importer(model.name, metadata, device)
        model.load_state_dict(torch.load(f"models_pkl/best_{model.name}_{dataset_train.name}.pth", weights_only=False))
