...
do
    ...
    do 
        ...
        do
            ...
            python make_samples.py --network $network --expl_method $expl_method --input_dataset $dataset --test_model
        done
    done
    for expl_method in CLIP-QDA-sample LIME_CBM SHAP_CBM
    do
        echo "$dataset CLIP-QDA $expl_method"
        python make_samples.py --network CLIP-QDA --expl_method $expl_method --input_dataset $dataset --test_model
    done

    for expl_method in LIME_CBM SHAP_CBM
    do
        echo "$dataset CBM-classifier-logistic $expl_method"
        python make_samples.py --network CBM-classifier-logistic --expl_method $expl_method --input_dataset $dataset --test_model
    done
