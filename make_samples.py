...
if __name__ == "__main__":
    ...
    def parse_args():
        ...
        parser.add_argument(
            ...
            type=str,
            default="resnet50",
            help="network (resnet50 or resnet50-bcos ViTB or ViTB-bcos or CLIP-QDA or CLIP-zero-shot or XNES-classifier-logistic)",
        )
        parser.add_argument("--gpus", type=str, default="0", help="gpu ids")  # TODO multigpu
        parser.add_argument(
            "--expl_method",
            type=str,
            default="explanation method (Rise_CBM, Xnesyl-Linear, LIME, SHAP, GradCAM, CLIP-QDA-sample, LIME_CBM, SHAP_CBM, AblationCAM, EigenCAM, EigenGradCAM, FullGrad, GradCAMPlusPlus, GradCAMElementWise, HiResCAM, ScoreCAM, XGradCAM)",
            help="gpu ids",
        )
        parser.add_argument(
            "--input_dataset",
    ...
    # Delete samples that have the same id_xai and dataset name
    listobj = utils.delete_id(list_xai_methods.index(expl_method_full), args.input_dataset, listobj)

    for data_sample in tqdm(dataset_sample):
        # Load sample
        ...
        if args.test_model:
            l_id_perturb = [3, 4]  # Test on perturbation == gaussian blur on another
        ...
        count_perturb = 0  # Counter of perturbations tried
        count_magn_perturb = 0  # Counter of magnutude of perturbations tried
