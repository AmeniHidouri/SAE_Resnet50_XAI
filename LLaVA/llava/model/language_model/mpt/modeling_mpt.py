...
class MPTModel(MPTPreTrainedModel):
    ...
    def _attn_bias(
        ...
        prefix_mask: torch.ByteTensor | None = None,
    ...
    def _apply_prefix_mask(self, attn_bias: torch.Tensor, prefix_mask: torch.Tensor):
        ...
        min_val = torch.finfo(attn_bias.dtype).min
        return attn_bias.masked_fill(cannot_attend, min_val)
    ...
    def forward(
        ...
        past_key_values: list[tuple[torch.FloatTensor]] | None = None,
        attention_mask: torch.ByteTensor | None = None,
        prefix_mask: torch.ByteTensor | None = None,
        sequence_id: torch.LongTensor | None = None,
        ...
        if self.alibi:
            ...
            pos = torch.arange(
                past_position, S + past_position, dtype=torch.long, device=input_ids.device
            ).unsqueeze(0)
            if attention_mask is not None:
                pos = torch.clamp(
