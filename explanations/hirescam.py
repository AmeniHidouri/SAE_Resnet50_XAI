...
from pytorch_grad_cam import HiResCAM as HiResCAM_

import utils


class HiResCAM:
    """Class that implements HiResCAM explanation method for a given model."""

    def __init__(self, model, device):
        """Initializes HiResCAM object.
        ...
        if model.name == "resnet50":
            ...
            self.explainer = HiResCAM_(model=model, target_layers=target_layers)
            ...
            print(f"Warning: HiResCAM_ is not implemented for model {model.name}.")

    def compute_and_plot_explanation(self, image, **kwargs):
        """Compute HiResCAM explanation and plot it.
