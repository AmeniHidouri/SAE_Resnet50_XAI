...
class GradCAM:
    """Class that implements GradCAM explanation method for a given model."""

    def __init__(self, model, device):
        """Initializes GradCAM_ object.
        ...
        if model.name == "resnet50":
            ...
            self.explainer = GradCAM_(model=model, target_layers=target_layers)
            ...
            print(f"Warning: GradCAM is not implemented for model {model.name}.")

    def compute_and_plot_explanation(self, image, **kwargs):
        """Compute GradCAM explanation and plot it.
