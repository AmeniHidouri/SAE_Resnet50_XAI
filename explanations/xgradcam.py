...
from pytorch_grad_cam import XGradCAM as XGradCAM_

import utils


class XGradCAM:
    """Class that implements XGradCAM explanation method for a given model."""

    def __init__(self, model, device):
        """Initializes XGradCAM object.
        ...
        if model.name == "resnet50":
            ...
            self.explainer = XGradCAM_(model=model, target_layers=target_layers)
            ...
            print(f"Warning: XGradCAM is not implemented for model {model.name}.")

    def compute_and_plot_explanation(self, image, **kwargs):
        """Compute XGradCAM explanation and plot it.
