...
from xplique.attributions import Occlusion as Occlusion_

import utils


class Occlusion:
    """Class representing Occlusion explanations."""
    ...
    def __init__(self, model):
        ...
        if model.name == "CLIP-QDA":
            self.explainer = Occlusion_(
                self.model.QDA, batch_size=1, patch_size=1, patch_stride=1, occlusion_value=12
            )
            ...
            self.explainer = Occlusion_(
                ...
                batch_size=1,
                patch_size=1,
                occlusion_value=self.model.mean_train,
            )
    ...
    def compute_and_plot_explanation(self, image, num_features=5, **kwargs):
        ...
        if save_activations:
            shap_values_abs = np.abs(vals)
            attributes_copy = self.model.list_concepts.copy()
            _, values_ordered, l_attributes_ordered = zip(
                *sorted(zip(shap_values_abs, vals, attributes_copy, strict=False)),
                strict=False,
            )
