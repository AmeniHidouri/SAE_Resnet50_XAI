...
from pytorch_grad_cam import AblationCAM as AblationCAM_
...
class AblationCAM:
    """Class that implements AblationCAM explanation method for a given model."""

    def __init__(self, model, device):
        """Initializes AblationCAM object.
        ...
        if model.name == "resnet50":
            ...
            self.explainer = AblationCAM_(model=model, target_layers=target_layers)
            ...
            target_layers = [model.vit_b_16.encoder.layers.encoder_layer_7.ln_2]
            self.explainer = AblationCAM_(
                model=model,
            ...
            target_layers = [model.clip_net.visual.transformer.resblocks[-2].ln_2]
            self.explainer = AblationCAM_(
                model=model,
                target_layers=target_layers,
                reshape_transform=reshape_transform,
            ...
            print(f"Warning: AblationCAM is not implemented for model {model.name}.")

    def compute_and_plot_explanation(self, image, **kwargs):
        """Compute AblationCAM explanation and plot it.
        ...
        # Compute AblationCAM explanation
