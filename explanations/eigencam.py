...
from pytorch_grad_cam import EigenCAM as EigenCAM_

import utils


class EigenCAM:
    """Class that implements EigenCAM explanation method for a given model."""

    def __init__(self, model, device):
        """Initializes EigenCAM_ object.
        ...
        if model.name == "resnet50":
            ...
            self.explainer = EigenCAM_(model=model, target_layers=target_layers)
            ...
            print(f"Warning: EigenCAM is not implemented for model {model.name}.")

    def compute_and_plot_explanation(self, image, **kwargs):
        """Compute EigenCAM explanation and plot it.
